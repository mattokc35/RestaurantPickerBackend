...
import { Restaurant, User } from "../server";
...
interface PlateBalanceGameData {
  players: Player[];
}

const plateBalanceGames: { [key: string]: PlateBalanceGameData } = {};

export const handlePlateBalanceGame = (
  io: Server,
  socket: Socket,
  sessions: any
) => {
  ...
  socket.on("plate-balance-finished", (timeBalanced: number) => {
    const sessionId = Array.from(socket.rooms).find(
      (room) => room !== socket.id
    );
    ...
    if (sessionId) {
      ...
      if (user) {
        ...
        let player = plateBalanceGames[sessionId].players.find(
          (p) => p.id === user.id
        );
        ...
        if (plateBalanceGames[sessionId].players.length === totalPlayers) {
          const sortedScores = plateBalanceGames[sessionId].players.sort(
            (a, b) => b.timeBalanced - a.timeBalanced // Sort by longest balance time
          );
          const winner = sortedScores[0];
          const winnerRestaurant = sessions[sessionId].restaurants.find(
            (r: Restaurant) => r.suggestedBy.id === winner.id
          );
          ...
          if (winnerRestaurant) {
            io.to(sessionId).emit(
              ...
              winner.username,
              (winner.timeBalanced / 1000).toFixed(2), // Send winner's time in seconds
              sortedScores
            );
          }
        }
  ...
  socket.on("disconnect", () => {
    ...
    if (sessionId && plateBalanceGames[sessionId]) {
      const indexOfPlayer = plateBalanceGames[sessionId].players.findIndex(
