...
interface Sessions {
  [key: string]: {
    host: User;
    guests: User[];
    restaurants: { name: string; suggestedBy: User }[];
  };
}
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("create-session", (sessionId: string) => {
    ...
    const hostUsername = generateUsername(socket.id);
